...
PaperStichは、各モニターに表示する画像をデスクトップ全体のレイアウトに合わせて1枚の壁紙へ合成し、その画像を壁紙として適用するアプリです。

モニターごとに画像を選び、全体の配置をプレビューしてから、ワンクリックで壁紙を適用できます。複数のモニターにまたがるパノラマ画像や、モニターごとに異なる画像を表示したい場合に利用できます。
...
配布用のアプリをビルドするには、以下を実行します。

```sh
npm run tauri build
```

ビルドされたパッケージは `src-tauri/target/release/bundle/` 以下に生成されます。現在はDebianパッケージ（`.deb`）が生成されます。
